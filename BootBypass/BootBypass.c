...
BOOLEAN StringToULONGLONG(PCWSTR str, ULONGLONG* out) {
    ...
    if (p[0] == L'0' && (p[1] == L'x' || p[1] == L'X')) {
        ...
        while (*p) {
            ...
            // Sprawdź overflow przed operacją
            if (result > (0xFFFFFFFFFFFFFFFF >> 4)) return FALSE;
        ...
        while (*p) {
            ...
            // Sprawdź overflow przed operacją
            if (result > (0xFFFFFFFFFFFFFFFF - digit) / 10) return FALSE;
...
// Parse string to unsigned long (supports hex 0x prefix) wrapper with a limitation to 32-bit
...
// ============================================================================
// REBOOT GUARDIAN SERVICE - Automatic reboot mechanism via service dependencies
// ============================================================================
...
BOOLEAN WriteMemory64(HANDLE hDriver, ULONGLONG address, ULONGLONG value, ULONG ioctl) {
    if (!WriteMemory32(hDriver, address, (ULONG)(value & 0xFFFFFFFF), ioctl)) return FALSE;
    if (!WriteMemory32(hDriver, address + 4, (ULONG)((value >> 32) & 0xFFFFFFFF), ioctl)) return FALSE;
    return TRUE;
}
...
ULONGLONG GetNtoskrnlBase(void) {
    ...
    if (moduleInfo->Count == 0) return 0;
    ...
    for (ULONG i = 0; i < moduleInfo->Count; i++) {
        ...
        if (isNtoskrnl) return (ULONGLONG)moduleInfo->Modules[i].ImageBase;
    }
...
// ============================================================================
// INI PARSER - Configuration file parsing and entry structure population
// ============================================================================

// Parse INI configuration and populate entry structures
ULONG ParseIniFile(PWSTR iniContent, PINI_ENTRY entries, ULONG maxEntries) {
    ULONG entryCount = 0;
...
__declspec(noreturn) void __stdcall NtProcessStartup(void* Peb) {
    ...
    entryCount = ParseIniFile(iniContent, entries, MAX_ENTRIES);
    ...
    DisplayMessage(L"\r\n====================================\r\n");
    DisplayMessage(L"All operations completed.\r\n");
    
	// Re-enable HVCI only if we actually performed patching operations
	if (!skipPatch) {
		RestoreHVCI();
